...
class ControlUnitDriver:
    ...
    def set_instruction(self, op_code, i_bit, d_bits=0):
        ...
        self.dut._log.info(f"Driving instruction: op={op_code}, i={i_bit}, d=0x{d_bits:X}")
...
async def test_reset_and_fetch_cycle(dut):
    ...
    await RisingEdge(dut.clk)
    assert dut.control_word.value == expected_F(5), "FETCH-T3 should assert F5 for direct instruction."
...
async def test_indirect_cycle(dut):
    ...
    await RisingEdge(dut.clk)
    assert dut.control_word.value == expected_F(4), "FETCH-T3 should assert F4 for indirect instruction."
    ...
    # T2 (NOP)
    await RisingEdge(dut.clk)
    assert dut.control_word.value == expected_F(), "INDIRECT-T2 should be a NOP (no F signals)."
